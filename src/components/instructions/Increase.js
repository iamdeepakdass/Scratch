...
export default function Increase({prevSize, id, index, uniqueId}) {
    ...
    useEffect(() => {
        dispatch(editInstruction({id, editInstructionIndex: index, instruction: {uniqueId, action: "increase", size: size}}));
    }, [size]);
    ...
    return (
        ...
        >
            <p>Increase size by <input type="number" value={size} onChange={(e) => setSize(e.target.value)} className="w-10 mx-1 text-black border border-black" /> times</p>
