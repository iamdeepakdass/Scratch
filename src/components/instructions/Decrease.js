...
export default function Decrease({prevSize, id, index, uniqueId}) {
    const [size, setSize] = useState(prevSize);
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(editInstruction({id, editInstructionIndex: index, instruction: {uniqueId, action: "decrease", size: size}}));
    }, [size]);
    ...
    return (
        ...
        >
            <p>Decrease size by <input type="number" value={size} onChange={(e) => setSize(e.target.value)} className="w-10 mx-1 text-black border border-black" /> times</p>
