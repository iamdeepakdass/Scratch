...
export default function GoTo({prevX, prevY, id, index, uniqueId}) {
    ...
    useEffect(() => {
        dispatch(editInstruction({id, editInstructionIndex: index, instruction: {uniqueId, action: "goTo", x: x, y: y}}));
    }, [x, y]); 
  return (
    <div 
    ...
    >
        <p>Goto x: <input type="number" value={x} onChange={(e) => setX(e.target.value)} className="w-10 mx-1 text-black border border-black" /> and y: <input type="number" value={y} onChange={(e) => setY(e.target.value)} className="w-10 mx-1 text-black border border-black" /></p>
