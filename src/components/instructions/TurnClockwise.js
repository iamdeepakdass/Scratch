...
export default function TurnClockwise({prevDegrees, id, index, uniqueId}) {
    const [degrees, setDegrees] = useState(prevDegrees);
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(editInstruction({id, editInstructionIndex: index, instruction: {uniqueId, action: "turnClockwise", degrees: degrees}}));
    }, [degrees]);
  return (
    <div 
    ...
    >
        <p>Turn Clockwise <input type="number" value={degrees} onChange={(e) => setDegrees(e.target.value)} className="w-10 mx-1 text-black border border-black" /> degrees</p>
