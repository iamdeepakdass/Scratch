...
export default function MoveY({prevSteps, id, index, uniqueId}) {
    const [steps, setSteps] = useState(prevSteps);
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(editInstruction({id, editInstructionIndex: index, instruction: {uniqueId, action: "moveY", steps: steps}}));
    }, [steps]);
    return (
        <div 
        ...
        >
            <p>Move <input type="number" value={steps} onChange={(e) => setSteps(e.target.value)} className="w-10 mx-1 text-black border border-black" /> steps in Y direction</p>
